...
import { dinnerRepository, seatRepository, userRepository, auditLogger } from "@dinewithme/db";
...
/**
 * Cancel a dinner and release all seats
 */
export async function cancelDinner(dinnerId: string): Promise<ActionResult> {
  try {
    const { userId: clerkUserId } = await auth();
    if (!clerkUserId) {
      return { success: false, error: "Unauthorized" };
